...
export const getAlbumInfo = async (artist: string, album: string): Promise<AlbumDetail | null> => {
  try {
    ...
    if (data && data.album) {
      ...
      const tracksWithArtistAndPlaycount = ensureArray(albumData.tracks?.track).map(
        (track: any, index: number) => ({
          ...track,
          artist: artistName,
          // Simulate a playcount relative to the album's total playcount and track rank
          playcount:
            Math.floor((Number(albumData.playcount) / (index + 5)) * (Math.random() * 0.4 + 0.8)) +
            1,
        })
      );
...
export const getArtistTopAlbums = async (artist: string): Promise<AlbumDetail[]> => {
  try {
    ...
    const albumsWithDetails = (await Promise.all(albumDetailsPromises)).filter(
      Boolean
    ) as AlbumDetail[];
