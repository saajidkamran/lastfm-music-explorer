...
const AlbumDetailView: React.FC = () => {
  const { selectedAlbum, loading, error, fetchArtistDetails, trackSortOption, setTrackSortOption } = useMusicStore();
  ...
  const sortedTracks = useMemo(() => {
    ...
    switch (trackSortOption) {
      ...
      case 'duration_asc':
        return tracksToSort.sort((a, b) => (parseInt(a.duration, 10) || 0) - (parseInt(b.duration, 10) || 0));
      case 'duration_desc':
        return tracksToSort.sort((a, b) => (parseInt(b.duration, 10) || 0) - (parseInt(a.duration, 10) || 0));
      case 'playcount_asc':
    ...
    <Flex flexDirection={{ base: 'column', md: 'row' }} gap={8}>
      <Box w={{ md: '33.333333%' }} flexShrink={0}>
        ...
        <Flex mt={4} justify="space-around" textAlign="center" fontSize="sm" bg="rgba(31, 41, 55, 0.5)" p={3} borderRadius="lg">
            ...
            <Box>
                <Text fontWeight="bold" fontSize="lg" color="pink.400">{Number(selectedAlbum.playcount).toLocaleString()}</Text>
      ...
      <Box w={{ md: '66.666667%' }}>
        <Flex align="flex-start" justify="space-between" gap={4}>
            <Box flex="1">
                <Text fontSize={{ base: '3xl', md: '5xl' }} fontWeight="extrabold" color="white">{selectedAlbum.name}</Text>
                <Text
        ...
        {selectedAlbum.tags.tag.length > 0 && (
          <Flex mt={4} flexWrap="wrap" gap={2}>
            {selectedAlbum.tags.tag.map(tag => (
              <Badge key={tag.name} bg="gray.700" color="pink.300" fontSize="xs" fontWeight="semibold" px={2.5} py={1} borderRadius="full">
                {tag.name}
              </Badge>
            ))}
          </Flex>
        ...
        {summary && (
            <Text mt={6} color="gray.300" lineHeight="relaxed" dangerouslySetInnerHTML={{ __html: summary }} />
        )}

        {tracks.length > 0 && (
          <Box mt={8}>
            <Text fontSize="2xl" fontWeight="bold" mb={4} color="gray.300" borderBottom="2px" borderColor="gray.700" pb={2}>Most Played Tracks</Text>
